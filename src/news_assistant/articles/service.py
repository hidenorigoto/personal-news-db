...
from ..core import DatabaseError, ArticleNotFoundError, ContentProcessingError
    ...
    def create_article_with_processing(self, db: Session, article_data: ArticleCreate) -> Article:
        ...
        try:
            ...
            try:
                ...
                # 処理結果で記事を更新
                db_article.title = processed_content.title
                db_article.summary = processed_content.summary
                db.commit()
                db.refresh(db_article)
                
            except ContentProcessingError as e:
                # コンテンツ処理に失敗した場合は元のデータで保存
                db_article.summary = article_data.summary or ""
                db.commit()
                db.refresh(db_article)
                # ログに記録（例外は再発生させない）
                import logging
