...
async def get_article(article_id: int, db: Session = Depends(get_db)) -> ArticleResponse:
    """特定の記事を取得"""
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Article with ID {article_id} not found"
        ) from e
...
async def delete_article(article_id: int, db: Session = Depends(get_db)) -> None:
    """記事を削除"""
    try:
