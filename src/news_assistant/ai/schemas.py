...
class AIConfig(BaseModel):
    ...
    api_key: Optional[str] = Field(None, description="API キー")
    base_url: Optional[str] = Field(None, description="ベースURL（カスタムエンドポイント用）")
...
class SummaryRequest(BaseModel):
    ...
    max_length: Optional[int] = Field(None, ge=50, le=2000, description="要約の最大文字数")
    language: str = Field(default="ja", description="要約言語（ISO 639-1）")
    custom_prompt: Optional[str] = Field(None, description="カスタムプロンプト")
...
class AIUsageStats(BaseModel):
    ...
    last_used: Optional[datetime] = Field(None, description="最終使用日時")
